        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    contained = self.ant
                    self.ant = insect
                    self.ant.contain_ant(contained) 
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0      
        while not nearest_place.bees or distance < self.min_range or distance> self.max_range:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            while behind_place.exit:
                if behind_place.ant:  ## double both guard and guarded
                    if isinstance(behind_place.ant, BodyguardAnt):
                        if isinstance(behind_place.ant, TankAnt) and behind_place.ant not in QueenAnt.strong_ants:
                            behind_place.ant.damage = behind_place.ant.damage *2
                            QueenAnt.strong_ants.append(behind_place.ant)
                        if behind_place.ant.ant and behind_place.ant.ant not in QueenAnt.strong_ants:
                            behind_place.ant.ant.damage = behind_place.ant.ant.damage *2
                            QueenAnt.strong_ants.append(behind_place.ant.ant)
                    else:
                        if behind_place.ant not in QueenAnt.strong_ants:  
                            behind_place.ant.damage = behind_place.ant.damage *2
                            QueenAnt.strong_ants.append(behind_place.ant)              
